            ...
            if len(bets["response"]) > 0:
                ...
                for bet in bets:
                    ...
                    if not match.is_reverse:
                        cfs_data = {
                            "1": round(1 + float(bet.get("team_1_cof", 0)), 3),
                            "2": round(1 + float(bet.get("team_2_cof", 0)), 3),
                        }
                    else:
                        cfs_data = {
                            "1": round(1 + float(bet.get("team_2_cof", 0)), 3),
                            "2": round(1 + float(bet.get("team_1_cof", 0)), 3),
                        }
                        if 'hc' in b_type:
                            side = 1 if side == 2 else 2
